...
/**
 * Stateless security for the Content service. Identity comes from the gateway-injected
 * headers ({@link HeaderAuthenticationFilter}); no session, no CSRF. Public reads
 * (browse posts/tags) stay open; premium-content gating is enforced in the service
 * layer (inter-service call), not by URL.
 */
@Configuration
@EnableWebSecurity
